    ...
    <update>
      <domain:update
       ...
       xsi:schemaLocation="urn:ietf:params:xml:ns:domain-1.0 domain-1.0.xsd">
        ...
        <domain:primNSip4>' . htmlspecialchars($nameservers['ip1'], ENT_QUOTES, 'UTF-8') . '</domain:primNSip4>
        <domain:secNSip4>'  . htmlspecialchars($nameservers['ip2'], ENT_QUOTES, 'UTF-8') . '</domain:secNSip4>
      </domain:update>
    </update>
    <clTRID>{clTRID}</clTRID>
  </command>
...
<epp xmlns="urn:ietf:params:xml:ns:epp-1.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="urn:ietf:params:xml:ns:epp-1.0 epp-1.0.xsd">
  <command>
    ...
    $contactData = array(
        ...
        'primNSip4'     => $nameservers['ip1'], # Primary Name Server IPv4
        'primNSip6'     => '', # Primary Name Server IPv6
        'secNSname'     => $nameservers['ns2'], # Secondary Name Server hostname
        'secNSip4'      => $nameservers['ip2'], # Secondary Name Server IPv4
    ...
    static public function extractNameservers($params)
    {
        ...
        for ($i = 1; $i <= 2; $i++) {
            if (!empty($params['ns' . $i])) {
                @list($ns, $ip) = explode('/', $params['ns' . $i], 2);

                if (empty($ip)) {
                    $ip = gethostbyname($ns);
                    if ($ip == $ns) {
                        throw new InvalidArgumentException("Please enter nameservers in the format 'ns.domain.com/12.34.56.78'");
                    }
                }
